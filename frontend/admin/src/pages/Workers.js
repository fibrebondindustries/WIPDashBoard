...
//     try {
//       setLoading(true);
//       const response = await axiosInstance.post("/api/departments/update-resources");
...
//             <h1 className="mb-5">Workers Management</h1>
//             <button
...
function Workers() {
  ...
  const [clearSelectedRows, setClearSelectedRows] = useState(false); // Clear rows after selection
  ...
  const handleUpdateLotQuantity = async () => {
    ...
    try {
      ...
      alert("Lot quantity updated successfully!");
      setShowModal(false);
      ...
      alert("Failed to update lot quantity. Please try again.");
    } finally {
      setLoading(false);
    }
  };
